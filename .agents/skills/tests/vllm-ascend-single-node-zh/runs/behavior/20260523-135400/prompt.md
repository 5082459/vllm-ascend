...
# 测试用例：单节点部署 - Qwen2.5（缺失部署章节）

## 测试场景

测试当模型教程文档缺失「Single-node Deployment」章节时，skill 应立即停止，不应生成部署目录。
...
- 不包含 `Single-node Deployment` 章节
- 部署章节为 `Online Inference on Multi-NPU`（单机多卡）和 `Offline Inference on Multi-NPU`
- 没有 `## Deployment` 下的 `### Single-node Deployment` 子章节

## 预期行为

Skill 应在步骤 3「检查部署模式支持」时：
- 检测到文档缺失 `Single-node Deployment` 章节
- 立即停止流程
- 输出错误信息：「模型教程缺失 Single-node Deployment 章节，无法生成部署包」
...
- 不能凭空生成不存在的部署脚本
- 不能使用其他章节（如 Online Inference on Multi-NPU）替代单节点部署
